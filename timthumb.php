...
$zoom_crop	= preg_replace( "/[^0-9]/", "", (isset($_REQUEST[ 'zc' ]) && !empty($_REQUEST[ 'zc' ])) ? $_REQUEST[ 'zc' ] : null );
...
check_cache( $cache_dir, $quality, $zoom_crop);
...
if(strlen($src) && file_exists( $src ) ) {
    ...
    show_image( $mime_type, $canvas, $quality, $cache_dir, $zoom_crop );
...
function show_cache_file ( $cache_dir,$quality, $zoom_crop ) {

    $cache_file = get_cache_file($quality, $zoom_crop);
    ...
    if( file_exists( $cache_dir . '/' . $cache_file ) ) {
        ...
        $thumbModified = strtotime($dateNow);
...
function get_cache_file ($quality, $zoom_crop) {
    $request_params = $_REQUEST;
    $cachename = $_REQUEST['src'] . $_REQUEST['w'] . $_REQUEST['h'] . $zoom_crop . $quality;
